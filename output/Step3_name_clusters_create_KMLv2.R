...
# Updated classification function using a threshold of 1.
# Any measurement less than 1 is considered equivalent to 0.
classify_site <- function(values, threshold = 3) {
...
site_class <- landsat_build3 %>% 
  ...
  group_by(site_num) %>% 
  arrange(year) %>% 
  summarize(site_type = classify_site(perc_of_max_3)) %>% 
